...
export function buildTFoots<Row, CellRenderer>(rendererModel: RendererModel<Row>, options?: Options<CellRenderer>) {
    ...
    if (hasChild) {
      ...
      if (foot != null) {
        tfoots.push({
          ...
          value: label,
          render: typeof cellRenderer === "function" ? cellRenderer(foot) : ({ cellProps }) => cellProps.value,
        });
      } else {
...
function buildColSpanQueue<Row>(
  rendererModel: RendererModel<Row>,
): Array<{ value: number | null; extends: boolean } | null> {
